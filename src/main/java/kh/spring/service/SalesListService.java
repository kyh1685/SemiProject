...
import kh.spring.dao.SalesListDAO;
...
public class SalesListService {
	...
	@Autowired
	private SalesListDAO sldao;
	public int insertSalesList(BuyTradeDTO dto) {
		return sldao.insertSalesList(dto);
	}
	public int deleteSalesList(SalesListDTO dto) {
		return sldao.deleteSalesList(dto);
	}
	public int delete(int goodSeq) {
		return sldao.delete(goodSeq);
	}
	public int insert(GoodsDTO dto) {
	      return sldao.insert(dto);
	   }
	public List<SalesListDTO> select(int page,SalesListDTO dto){
		return sldao.select(page,dto);
	}
	public int totalCount(SalesListDTO dto) throws Exception{
		return sldao.totalCount(dto);
	}
	public String navi(int currentPage,SalesListDTO dto) throws Exception {
		...
		if(needPrev) {
			sb.append("<a href='/mypage/salesList.mp?page="+(startNavi-1)+"'> < </a>");
		}
		for(int i = startNavi; i<=endNavi; i++) {
			sb.append("<a href='/mypage/salesList.mp?page="+i+"'>"+i+" "+"</a>");
		}
		if(needNext) {
			sb.append("<a href='/mypage/salesList.mp?page="+(endNavi+1)+"'> > </a>");
		}
		return sb.toString();
	}
	public List<SalesListDTO> search(int page, String startDate, String endDate,SalesListDTO dto) {
		...
		return sldao.search(page,startDate,endDate,dto);
	}
	public int searchTotalCount(String startDate, String endDate,SalesListDTO dto) throws Exception{
		...
		int year = Integer.parseInt(arr[0]);
		endDate = UtilTime.Check(year, month, day);
		return sldao.searchTotalCount(startDate, endDate,dto);
	}
	public String searchNavi(int currentPage,String startDate,String endDate,SalesListDTO dto) throws Exception {
		String arr[] = endDate.split("-");
		...
		if(needPrev) {
			sb.append("<a href='/mypage/searchSalesList.mp?page="+(startNavi-1)+"&startDate="+startDate+"&endDate="+endDate+"'> < </a>");
		}
		for(int i = startNavi; i<=endNavi; i++) {
			sb.append("<a href='/mypage/searchSalesList.mp?page="+i+"&startDate="+startDate+"&endDate="+endDate+"'>"+i+" "+"</a>");
		}
		if(needNext) {
			sb.append("<a href='/mypage/searchSalesList.mp?page="+(endNavi+1)+"&startDate="+startDate+"&endDate="+endDate+"'> > </a>");
		}
		return sb.toString();
	}
}
