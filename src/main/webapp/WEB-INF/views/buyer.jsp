...
<body>
	...
	<div class="container">
		...
		<div class="inputArea3">
			<c:choose>
				<c:when test="${buyer_req == 'N'}">
					<form action="/trade/buyer_cfming.pg" method="get">
						<input type=hidden name=seq value=${goodSeq}>
						<button type="submit" id="insertBtn" class="btn">거래요청</button>
					</form>
				</c:when>
				<c:when test="${seller_cfm == 'Y' }">
					<form action="/trade/buyer_cfmComplete.pg?" method="get">
						<input type=hidden name=seq value=${goodSeq}>
						<button type="submit" id="insertBtn" class="btn">거래완료</button>
					</form>
				</c:when>
				<c:when test="${buyer_req == 'Y' }">
					<form action="/trade/buyer_cfmDelete.pg" method="get">
					<input type=hidden name=seq value=${goodSeq}>
						거래를 요청중입니다.
