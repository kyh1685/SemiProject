	...
	<div class="container">
		<div class="inputArea2">
			<div class="inputArea2_1">
				[상품이미지/상품정보]<br>
				<img src="/files/${dto.goodImg}" width=450 height=320>
			</div>
			<div class="inputArea2_2">
				...
				<div class="inputArea2_2_2">
					[거래요청자 ID]<br> ${dto.goodWriter}
				</div>
				<div class="inputArea2_2_3">
					[거래요청자 이메일]<br> ${mdto.email }
				</div>
				<div class="inputArea2_2_4">
					[거래요청자 전화번호]<br> ${mdto.contact}
				</div>
				<div class="inputArea2_2_5">
					[판매 금액]<br> ${dto.goodPrice }원
				</div>
			</div>
		</div>
		<div class="inputArea3">
			<c:choose>
				<c:when test="${req == 'N'}">
					<form action="/trade/req_trade.pg" method="get">
						<input type=hidden name=seq value=${dto.goodSeq}>
						<button type="submit" id="insertBtn" class="btn">거래요청</button>
					</form>
				</c:when>
				<c:when test="${cf == 'Y' }">
					<form action="/trade/complete_trade.pg?" method="get">
						<input type=hidden name=seq value=${dto.goodSeq}>
						<button type="submit" id="insertBtn" class="btn">거래완료</button>
					</form>
				</c:when>
				<c:when test="${req == 'Y' }">
					<form action="/trade/cancelReq_trade.pg" method="get">
					<input type=hidden name=seq value=${dto.goodSeq}>
					<input type=hidden name=buyer value=${id}>
						거래를 요청중입니다.
						<button type="submit" id="deleteBtn" class="btn">요청취소</button>
					</form>
				</c:when>
				<c:otherwise>
					거래할 상품이 존재 하지 않습니다.
